...
class RequestDAOImpl(RequestDAO):
    ...
    def get_request_with_id(cls, employee_id, request_id):
        ...
        cursor.execute(sql)
        records = cursor.fetchall()
        requests = [Request(elem[0], elem[1], (elem[2]), (elem[3]), int(elem[4]), (elem[5]), (elem[6]), (elem[7]), int(elem[8]), (elem[9]), (elem[10]), (elem[11]), (elem[12]), (elem[13]), int(elem[14])) for elem in records]
        return requests
    ...
    def update_request_with_id(cls, employee_id, request_id, req_funds, added_info, is_denied, denied_reason,
        ...
        sql = f"UPDATE requests SET req_funds ={req_funds}, added_info ={added_info}, is_denied ={is_denied}," \
              f" denied_reason ={denied_reason}, is_approved ={is_approved}, phase ={phase} WHERE request_id = {request_id}" \
